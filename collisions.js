...
class Test_Data
{ constructor( context )
    { this.textures = { rgb   : context.get_instance( "/assets/drake.jpg"   ),
                        earth : context.get_instance( "/assets/drake1.jpg" ),
                        grid  : context.get_instance( "/assets/drake2.jpg"  ),
                        stars : context.get_instance( "/assets/image1.jpeg" ),
                        text  : context.get_instance( "/assets/kanye.jpg"  )
                      }
      this.shapes = { //donut  : new Torus          ( 15, 15 ),
                      //cone   : new Closed_Cone    ( 4, 10 ),
                      //capped : new Capped_Cylinder( 4, 12 ),
                      ball   : new Subdivision_Sphere( 2 )
                /*      cube   : new Cube(),
  ...
  random_shape( shape_list = this.shapes )    //Not using in main code
...
class Collision_Demo extends Drake_Concert_Scene    // Demonstration: Detect when some flying objects collide with one another, coloring them red.
{ constructor(  context, control_box )
    { super(    context, control_box );
      if( !context.globals.has_controls   )
        context.register_scene_component( new Movement_Controls( context, control_box.parentElement.insertCell() ) );
      ...
      context.globals.graphics_state.    camera_transform = Mat4.translation([ 0,0,-50 ]); //Camera start position
      context.globals.graphics_state.projection_transform = Mat4.perspective( Math.PI/4, context.width/context.height, 1, 500 );
  ...
  update_state( dt, num_bodies = 40 )
    { if   ( this.bodies.length > num_bodies )  this.bodies = this.bodies.splice( 0, num_bodies );                // Max of 20 bodies
      while( this.bodies.length < num_bodies )                                        // Generate moving bodies:
        this.bodies.push( new Body( this.data.random_shape(), undefined, Vec.of( 1,1,1 ) ) //Makes a sphere in our current version
